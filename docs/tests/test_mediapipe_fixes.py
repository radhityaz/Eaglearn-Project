...
from mediapipe_processors import (
    FaceMeshProcessor,
    PoseProcessor,
    ...
    try:
        ...
        for i, frame in enumerate(test_frames):
            ...
            try:
                ...
                logger.debug(
                    f"Frame {i}: face_detected={face_result.get('face_detected', False)}"
                )
            ...
            try:
                ...
                logger.debug(
                    f"Frame {i}: body_detected={pose_result.get('body_detected', False)}"
                )
            except Exception as e:
...
def test_consecutive_error_handling():
    ...
    try:
        ...
        for i in range(10):
            ...
            result = face_processor.process(invalid_frame, mock_state)
            logger.debug(
                f"Error frame {i + 1}: consecutive_errors={getattr(face_processor, 'consecutive_errors', 'N/A')}"
            )
    ...
    try:
        ...
        for i, problematic_input in enumerate(problematic_inputs):
            ...
            assert result["emotion"] == "neutral", (
                f"Should return neutral for input {i + 1}"
            )
