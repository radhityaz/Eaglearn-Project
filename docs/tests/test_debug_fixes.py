...
def test_deepface_tuple_error():
    ...
    try:
        ...
        logger.info(
            f"Result for valid frame: emotion={result.get('emotion', 'N/A')}, method={result.get('method', 'N/A')}"
        )
...
def test_face_processing_arithmetic():
    ...
    try:
        ...
        logger.info(
            f"Face processing result: face_detected={result.get('face_detected', False)}"
        )
        ...
        if hasattr(mock_state, "calibration_gaze_offset_x"):
            logger.info(
                f"calibration_gaze_offset_x type: {type(mock_state.calibration_gaze_offset_x)}"
            )
...
def test_mediapipe_timestamp_errors():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Pose processing result: body_detected={pose_result.get('body_detected', False)}"
            )
        except Exception as e:
    ...
    try:
        ...
        for i in range(10):
            ...
            try:
                ...
                logger.debug(
                    f"Frame {i + 1}: face_detected={face_result.get('face_detected', False)}"
                )
            except Exception as e:
            ...
            if i % 3 == 0:  # Test every few frames
                try:
                    ...
                    logger.debug(
                        f"Frame {i + 1}: emotion={emotion_result.get('emotion', 'N/A')}"
                    )
                except Exception as e:
